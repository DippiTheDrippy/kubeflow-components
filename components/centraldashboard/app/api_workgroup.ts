...
import {
  Binding as WorkgroupBinding,
  DefaultApi,
} from "./clients/profile_controller";
 ...
 */
export function mapWorkgroupBindingToSimpleBinding(
  bindings: WorkgroupBinding[],
): SimpleBinding[] {
 ...
 */
export function mapNamespacesToSimpleBinding(
  user: string,
 ...
 */
export function mapSimpleBindingToWorkgroupBinding(
    ...
    return Router()
      .get("/exists", async (req: Request, res: Response) => {
        try {
          ...
          if (req.user.hasAuth) {
            ...
            response.hasWorkgroup = !!(workgroup.namespaces || []).find(
              (w) => w.role === "owner",
            );
          } else {
            // Basic auth workgroup condition
            response.hasWorkgroup = !!(
              await this.getAllWorkgroups(req.user.username)
            ).length;
          }
          res.json(response);
        } catch (err) {
      ...
      .post("/create", async (req: Request, res: Response) => {
        ...
        try {
          ...
          const PROFILE_PREFIX = "kubeflow-user-ns-";

          function normalizeProfileNamespace(input: string): string {
            const suffix = input.startsWith(PROFILE_PREFIX)
              ? input.slice(PROFILE_PREFIX.length)
              : input;

            return `${PROFILE_PREFIX}${suffix}`
              .replace(/[^a-z0-9-]/g, "-")
              .replace(/-+/g, "-")
              .replace(/^-+|-+$/g, "")
              .toLowerCase();
          ...
          const requestedNamespace = profile.namespace || req.user.username;
          const namespace = normalizeProfileNamespace(requestedNamespace);
      ...
      .get("/env-info", async (req: Request, res: Response) => {
        try {
          ...
          console.log(
            `Unable to get environment info: ${error}${err.stack ? "\n" + err.stack : ""}`,
          );
      ...
      .use((req: Request, res: Response, next: NextFunction) => {
        if (!req.user.hasAuth) {
          return apiError({
            ...
            code: 405,
            error:
              "Unable to ascertain user identity from request, cannot access route.",
          });
        }
        next();
      })
      .delete("/nuke-self", async (req: Request, res: Response) => {
        try {
          ...
          const { body: serverBody } = await this.profilesService.deleteProfile(
            namespace,
            { headers },
          );
          res.json({
            message: `Removed namespace/profile ${namespace}`,
            serverBody,
          });
        } catch (err) {
          surfaceProfileControllerErrors({
