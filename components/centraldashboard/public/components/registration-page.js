...
export class RegistrationPage extends utilitiesMixin(PolymerElement) {
  static get template() {
    const vars = { logo };
    return html([`<style>${css.toString()}</style>${template(vars)}`]);
  }
  ...
  static get properties() {
    return {
      ...
      namespacePrefix: { type: String, value: "kubeflow-user-ns-" },
  ...
  _onUserDetails(d) {
    this.namespaceSuffix = this.userDetails
  ...
  getNamespaceName() {
    return `${this.namespacePrefix}${this.namespaceSuffix}`;
  }
  validateNamespace() {
    const finalRgx = /^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/;
    if (finalRgx.test(this.getNamespaceName())) return true;
  ...
  async finishSetup() {
    ...
    API.body = { namespace: this.getNamespaceName() };
